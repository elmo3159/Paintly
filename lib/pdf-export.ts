...
export async function exportSingleGenerationToPdf(
  ...
  try {
    ...
    const imageHeight = 80 // 画像の高さ（mm）
    doc.addImage(originalBase64, 'JPEG', margin, yPos, contentWidth, imageHeight)
    yPos += imageHeight + 10
    ...
    doc.addImage(generatedBase64, 'JPEG', margin, yPos, contentWidth, imageHeight)
...
export async function exportMultipleGenerationsToPdf(
  ...
  try {
    ...
    for (let i = 0; i < dataList.length; i++) {
      ...
      const originalBase64 = await imageUrlToBase64(data.originalUrl)
      doc.addImage(originalBase64, 'JPEG', margin, yPos, imageWidth, imageHeight)

      // 生成画像
      doc.text('シミュレーション結果', margin + imageWidth + 5, yPos - 4)

      const generatedBase64 = await imageUrlToBase64(data.generatedUrl)
      doc.addImage(generatedBase64, 'JPEG', margin + imageWidth + 5, yPos, imageWidth, imageHeight)
    }
